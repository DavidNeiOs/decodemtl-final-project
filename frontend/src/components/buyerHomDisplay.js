...
class BuyerHomeDisplay extends Component {
  ...
  formatItems = name => {
    if (name === "") {
      let filteredList = this.state.currItems
        .filter(item => item.state === "TO_AUCTION")
        .map(i => {
          return (
            <Card>
              <Image
                ...
                >
                  ...
                  <Modal.Content image scrolling>
                    ...
                    <Modal.Description>
                      ...
                      <BidLog
                        itemId={i.itemId}
                        userBid={this.props.usr.username}
                        onUpdateBid={this.handleUpdateBid}
                      />
                    </Modal.Description>
      ...
      return this.state.activeItem
        ? rItems.filter(item => item.category === this.state.activeItem)
        : rItems;
    } else {
      ...
      let filteredList = this.state.currItems
        .filter(
          item =>
            ...
            (item.state === "TO_AUCTION" && item.orgId === category)
        )
        .map(i => {
          return (
            <Card>
                ...
                >
                  ...
                  <Modal.Content image scrolling>
                    ...
                    <Modal.Description>
                      ...
                      <p>{i.description}</p>
                      <h2>
                        Latest Bid at {this.state.currBid || i.lastPrice} $
                      </h2>
                      <br />
                      <h2>
                        <Timer endDate={i.bidFinDate} item={i} />
                    ...
                    <Modal.Description align="center">
                      <ItemOrgCard chosenOrgId={i.orgId} />
                      <Segment.Group raised>
                        <Segment inveted color="black" align="center">
                          <Header>Chat</Header>
    ...
    return (
      <Grid>
        ...
        <Grid.Column stretched width={12}>
          <Segment>
            <Grid columns={5}>{this.formatItems(this.state.activeItem)}</Grid>
          </Segment>
        </Grid.Column>
      </Grid>
    );
