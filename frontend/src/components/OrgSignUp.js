...
class OrgSignUp extends Component {
  ...
  handleChange(evt, { name, value }) {
    // *** if uploading the logo or banner change state acordingly ***
  ...
  handleSubmit() {
    // *** check both password inputs contain the same info ***
    if (this.state.password !== this.state.confirmPassword) {
      this.setState({ password: "", confirmPassword: "" });
      alert(`passwords do not match`);
      return;
    }
    // *** make a copy of the state and send it to backend ***
    ...
    fetch("/signUp", {
      ...
      .then(() => {
        ...
        // *** once the account is created log in automatically ***
        fetch("/logIn", {
          method: "POST",
          mode: "same-origin",
          credentials: "include",
