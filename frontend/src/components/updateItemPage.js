...
/**
 * this Component renders a form in case a non-profit
 * wants to change any information of any item they are
 * auctioning
 */
class UpdateItemPage extends Component {
  ...
  handleSubmit() {
    // *** make a copy to the state and add extra values needed ***
    let state = Object.assign({}, this.state);
    state["orgId"] = this.props.orgId;
    state["username"] = this.props.org[0].username;
    // *** send info back to server to update info there ***
    fetch("/updateItem", {
      method: "PUT",
      mode: "same-origin",
      credentials: "include",
    ...
    // *** change values in store to display the right view ***s
    this.props.dispatch({
      type: "showOrgPage",
  ...
  componentDidMount() {
    // *** set the state with the values of element to change ***
    this.setState({
