...
class BuyerProfile extends Component {
  ...
  /**
   * Send credential to server for reconigtion
   * then get and show the items related to this specific user
   */
  getUserPro() {
    fetch("/getUserProfile", {
      ...
      credentials: "include",
      body: JSON.stringify({
        username: this.props.byr.username,
        userId: this.props.byrId
      })
    })
      ...
      .then(responseBody => {
        ...
        this.setState({
          wonItems: body.wonItems,
          lostItems: body.lostItems,
    ...
    return (
      <div>
        ...
        <Grid relaxed="very" columns={4}>
          {this.formatItems(this.state.wonItems).map(item => (
        ...
        <Grid relaxed="very" columns={4}>
          {this.formatItems(this.state.lostItems).map(item => (
            <Grid.Column>{item}</Grid.Column>
          ))}
        </Grid>
        ...
        <Grid relaxed="very" columns={4}>
          {this.formatItems(this.state.transactions).map(item => (
            <Grid.Column>{item}</Grid.Column>
          ))}
        </Grid>
        <br />
