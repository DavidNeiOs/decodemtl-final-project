...
app.get("/getParams", (req, res) => {
  try {
    if (
      !(
        serverState.categoriesList.length > 0 &&
...
app.post("/signUp", (req, res) => {
  try {
    ...
    if (
      ...
      collOrg
        .find({ website: bodyParam.website })
          ...
          if (result.length > 0) {
            res.send(
              JSON.stringify({
                status: false,
                message:
                  "error in account creation, website alrready registered"
              })
            );
          } else {
            collOrg
              .find({ username: bodyParam.username })
              .toArray(function(err, result) {
                if (err) {
                  throw err;
                }
                ...
                if (result.length > 0) {
                  res.send(
                    JSON.stringify({
                      status: false,
                      message:
                        "error in account creation, username alrready used"
                    })
                  );
                } else {
                  ...
                  collOrg.insertOne(bodyParam, function(err, result) {
                    ...
                    collOrg.updateOne(myquery, newvalues, function(
                      err,
                      result
                    ) {
                      if (err) {
        ...
        .toArray(function(err, result) {
          ...
          if (result.length > 0) {
            res.send(
              JSON.stringify({
                status: false,
                message: "error in account creation, username alrready used"
              })
            );
          } else {
            //delete userType attribute
...
app.post("/updateInfo", (req, res) => {
  ...
  if (!params.userId && !params.orgId)
    return res.send(JSON.stringify({ status: "No ID provided!" }));
  const collection = params.orgId
    ? datab.collection(collOrganizations)
    : datab.collection(collBuyers);
  ...
  try {
    ...
    collOrg
      .find({
        ...
        password: sha256(bodyParam.password)
      })
      .toArray(function(err, result) {
        if (err) {
          throw err;
    ...
    collSess.find(query).toArray(function(err, result) {
      ...
      if (result.length > 0) {
        ...
        res.send(
          JSON.stringify({
            status: false,
            message: "user does not have any active session"
          })
        );
      }
    });
  ...
  try {
    ...
    collSess.find(querySess).toArray(function(err, result) {
      if (err) {
        ...
        let newvalues = {
          $set: {
            bidCancelDate: new Date().toISOString(),
            state: ITEM_STATE_CANCELED
          }
        };
        //update Item
        collItem.updateOne(myquery, newvalues, function(err, result) {
          if (err) {
            ...
            res.send(
              JSON.stringify({
                status: false,
                message: "error trying to cancel the item"
              })
            );
          }
        });
  ...
  try {
    ...
    collSess.find(querySess).toArray(function(err, result) {
      if (err) {
        ...
        collItm
          .find({ itemId: bodyParam.itemId, state: "TO_AUCTION" })
          .toArray(function(err, result) {
            if (err) {
              throw err;
            } else if (result.length > 0) {
              ...
              collBid.insertOne(bodyParam, function(err, result) {
                if (err) {
                  res.send(
                    JSON.stringify({
                      status: false,
                      message: "error processing bid to item"
                    })
                  );
                  throw err;
                }
                ...
                collBid.updateOne(myquery, newvalues, function(err, result) {
                  if (err) {
                    ...
                    collItm.updateOne(
                      { itemId: bodyParam.itemId },
                      { $set: { lastPrice: bodyParam.bid } },
                      function(err, result) {
                        if (err) {
                          throw err;
                        } else if (result.result.nModified > 0) {
              ...
              res.send(
                JSON.stringify({
                  status: false,
                  message: "item not found, offer can not be processed"
                })
              );
            }
          });
              ...
              collBid
                ...
                .toArray(function(err, result) {
                  ...
                  if (result.length > 0) {
                    ...
                    let newvalues = {
                      $set: {
                        bidClosedDate: new Date().toISOString(),
                        state: ITEM_STATE_AUCTIONED,
                        lastPrice: result[0].bid,
                        winnerUserId: userWinner
                      }
                    };
                    collItem.updateOne(myquery, newvalues, function(
                      err,
                      result
                    ) {
                      if (err) {
                        ...
                        res.send(
                          JSON.stringify({
                            status: false,
                            message: "error trying to close the item"
                          })
                        );
                      }
                    });
  ...
  try {
    ...
    let cb = () => {
      ...
      if (processSearchWonItems && processSearchItemsBidded) {
        ...
        collBid
          .find({ itemId: { $nin: arrayWinners }, userId: bodyParam.userId })
          .toArray(function(err, result) {
            if (err) {
              throw err;
            } else if (result.length > 0) {
    ...
    collSess.find(querySess).toArray(function(err, result) {
      if (err) {
        ...
        //won items
        collItem
          .find({ winnerUserId: bodyParam.userId })
          .toArray(function(err, result) {
            if (err) {
              throw err;
            } else if (result.length > 0) {
...
function getSessionIdFromCookie(req) {
  try {
    let sessionID =
      req.headers.cookie != undefined ? req.headers.cookie.split("=")[1] : "";
    return sessionID;
  } catch (error) {
...
function closeItemProcess(itemIdParam) {
  try {
    ...
    //verify item is not closed/auctioned
    collItem
      .find({ itemId: itemIdParam, state: "TO_AUCTION" })
      .toArray(function(err, result) {
        if (err) {
          throw err;
        }
        ...
        result.forEach(e => {
          if (e.itemId === itemIdParam) {
            ...
            mailData.html =
              ...
              " " +
              winner.lastName +
              "</b> the auction has finished and you are the happy winner of the item" +
              e.title +
              " with a bid of  " +
              winner.biddedPrice +
              ". Please contact with the non-profit org responsible: <b>" +
              e.orgDetail[0].orgName +
              "</b>. Contact email:" +
              e.orgDetail[0].email +
              "</p>";
        ...
        result.forEach(e => {
          if (e.itemId === itemIdParam) {
            ...
            mailData.html =
              "<p>Estimated <b>" +
              e.orgDetail[0].orgName +
              "</b> the auction for the item " +
              e.title +
              " has been finished and there was no winner.</p>";
            sendEmailProcess(mailData);
          }
        });
      }
